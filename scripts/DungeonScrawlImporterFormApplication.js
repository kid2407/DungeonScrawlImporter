export class DungeonScrawlImporterFormApplication extends FormApplication {
    ...
    async parseShapes(shapes, doorIDs, stairsIds, gridCellSize, offsetX, offsetY) {
        ...
        let calculatedXOffset = sceneCellSize * offsetX + mostNegativeX * -1 + sceneDimensions.sceneX;
        let calculatedYOffset = sceneCellSize * offsetY + mostNegativeY * -1 + sceneDimensions.sceneY;
    ...
    async processLights(lightData, offsetData, gridCellSize) {
        ...
        for (let key in lightData) {
            ...
            let light = {
                x:      data.transform[4] * (game.scenes.current.grid.size / gridCellSize) + offsetData.xOffset,
                y:      data.transform[5] * (game.scenes.current.grid.size / gridCellSize) + offsetData.yOffset,
                config: {
                    alpha:      data.alpha,
        ...
        await game.scenes.current.createEmbeddedDocuments("AmbientLight", lights)
    }
